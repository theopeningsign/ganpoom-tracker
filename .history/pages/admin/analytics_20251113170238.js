...
export default function AnalyticsPage() {
  ...
  const setQuickDateRange = (type) => {
    ...
    const month = today.getMonth() // 0-based (11월 = 10)
    ...
    switch (type) {
      case 'thisMonth':
        // 이번 달 1일부터 오늘까지
        startDate = new Date(year, month, 1) // 이번 달 1일
        endDate = new Date(year, month, date) // 오늘
        break
      case 'lastMonth':
        // 지난 달 1일부터 지난 달 말일까지
        ...
        startDate = new Date(lastMonthYear, lastMonthIndex, 1) // 지난 달 1일
        endDate = new Date(year, month, 0) // 지난 달 마지막 날
        break
      case 'last3Months':
        // 3개월 전 1일부터 오늘까지
        const threeMonthsAgo = month - 2
        const threeMonthsYear = threeMonthsAgo < 0 ? year - 1 : year
        const threeMonthsIndex = threeMonthsAgo < 0 ? 12 + threeMonthsAgo : threeMonthsAgo
        startDate = new Date(threeMonthsYear, threeMonthsIndex, 1)
