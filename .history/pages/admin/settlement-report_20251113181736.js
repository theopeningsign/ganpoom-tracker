...
export default function SettlementReportPage() {
  ...
  const loadReportData = () => {
    try {
      ...
      // 최근 4개월 데이터 생성 (현재 정산월 포함)
      for (let i = 3; i >= 0; i--) {
        const targetMonth = currentMonth - i
        const targetYear = targetMonth <= 0 ? currentYear - 1 : currentYear
        const adjustedMonth = targetMonth <= 0 ? targetMonth + 12 : targetMonth
        
        const monthStr = `${targetYear}-${adjustedMonth.toString().padStart(2, '0')}`
        
        // 해당 월의 통계 계산
        const monthlyStats = calculateMonthlyStats(adjustedMonth)
        
        reports.push({
          month: monthStr,
          monthName: `${adjustedMonth}월`,
          year: targetYear,
          isCurrentSettlement: i === 0, // 가장 최근 월이 현재 정산 대상
          ...monthlyStats
        })
      }
