...
export default function AgentsPage() {
  ...
  // localStorage에서 에이전트 불러오기
  const loadFromLocalStorage = () => {
    try {
      const saved = localStorage.getItem('mockAgents')
      if (saved) {
  ...
  // localStorage에 에이전트 저장
  const saveToLocalStorage = (agentsList) => {
    try {
      localStorage.setItem('mockAgents', JSON.stringify(agentsList))
    } catch (error) {
                      ...
                      onMouseOut={(e) => e.target.style.background = 'white'}>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: '20px' }}>
                          ...
                          <div style={{ textAlign: 'right' }}>
                            <div style={{ fontSize: '1.1rem', fontWeight: 'bold', color: '#28a745', marginBottom: '8px' }}>
                              {formatCurrency(agent.commission || 0)}
                            </div>
