                ...
                return (
                  ...
                  >
                    ...
                    <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                      ...
                      {selectedChannel === 'unattributed' && ev.referrer && (
                        <a href={ev.referrer} target="_blank" rel="noopener noreferrer"
                          onClick={e => e.stopPropagation()}
                          style={{ fontSize: 11, color: '#4facfe', textDecoration: 'none', wordBreak: 'break-all' }}
                          title={ev.referrer}
                        >🔗 {shortReferrer(ev.referrer)}</a>
                      )}
                    </div>
                  </div>
                )
          ...
          {detailTab === 'referrers' && (
            <div>
              ...
              {!detail.referrerDomains || detail.referrerDomains.length === 0 ? (
                ...
                const max = Math.max(...detail.referrerDomains.map(d => d.visits), 1)
                return detail.referrerDomains.map((item) => {
                  ...
                  return (
                    <div key={item.domain} style={{ marginBottom: 14 }}>
                      ...
                      <div style={{ height: 6, background: '#f0f0f0', borderRadius: 3 }}>
                        <div style={{
                          height: '100%', width: `${pct}%`,
                          background: item.isDirect ? '#bdc3c7' : '#e67e22',
                          borderRadius: 3, transition: 'width 0.4s'
                        }} />
                      </div>
                    </div>
